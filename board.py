            ...
            for j in range(0, 8):
                if (i+j) % 2 == 1:  # (i+j)%2==1 means a black case on the board
                    if i < 3:  # at start of game all black pieces are on this side
                        # 12 pieces for player 1
                        self.case_list.append(check(1, 0, [i, j], self))
                    elif i > 4:
                        # 12 pieces for player 2, i used 1 and -1 as the ID of the players to make coding x-axis movements easier
                        self.case_list.append(check(-1, 0, [i, j], self))

    # This function tells us if a position on the board (ex: 1,2) is clear.
    def is_clear(self, pos1, pos2):
        for i in self.case_list:
            if i.pos == [pos1, pos2]:  # means a checkers piece is in this position
                return False
        return True

    # opposite of function above, returns the checkers piece object
    def get_index(self, pos1, pos2):
        for i in range(0, len(self.case_list)):
            # a checkers piece with this position exists, we return the checkers piece object
...
class check:
    # properties i used for a checkers piece are the player ID, if its a king (1 or 0), its position and the board its on
    def __init__(self, player, king, pos, board):
        self.player = player
        self.king = king
        self.pos = pos
        self.board = board
            ...
            if (self.pos[0]-2*self.player >= 0 and self.pos[0]-2*self.player < 8):
                if self.pos[1]+2 < 8:
                    if self.board.is_clear(self.pos[0]-2*self.player, self.pos[1]+2):
                        if self.player == -1*self.board.player(self.pos[0]-self.player, self.pos[1]+1):
                            l.append(
                                [self.pos[0]-2*self.player, self.pos[1]+2])
                            ll.append([self.pos[0]-self.player, self.pos[1]+1])
                if self.pos[1]-2 >= 0:
                    if self.board.is_clear(self.pos[0]-2*self.player, self.pos[1]-2):
                        if self.player == -1*self.board.player(self.pos[0]-self.player, self.pos[1]-1):
...
# checks if the simulation should be performed as a king or not (if the new position is at the edge of the board or if the checkers piece have moved to it before)
        if self.king == 0:
            if self.player == 1:
                if pos[0] == 7:
                    new_list[self.board.get_index(
                        self.pos[0], self.pos[1])].king = 1
            if self.player == -1:
                if pos[0] == 0:
                    new_list[self.board.get_index(
                        self.pos[0], self.pos[1])].king = 1
                ...
                if pos[1]-2 >= 0:
                    if board_index(new_list, [pos[0]-2*self.player, pos[1]-2]) == [13, 0]:
                        if self.player == -1*board_index(new_list, [pos[0]-self.player, pos[1]-1])[1]:
                            l.append([pos[0]-2*self.player, pos[1]-2])
                            ll.append([pos[0]-self.player, pos[1]-1])
        # return the possible positions within one move and the captured pieces within one move
        return [l, ll]
        ...
        while next != len(l):
            ...
            if current_list[0] != []:
                for j in range(0, len(current_list[0])):
                    fp = current[2]+[[current_list[1]
                                      [j][0], current_list[1][j][1]]]
                    l.append([current_list[0][j], current[1]+str(jiji), fp])
                    jiji = jiji + 1
            next = next + 1
        return l

    # checks the max number of moves i can make in the paths returned by the function above
    ...
    # turns the list of indexes into a list of lists of index of the same length
    ...
    # turns an index into a list of indexes ex : 10102 -> [1,10,101,1010,10102]
    def pathify(self, id):
        l = []
        ...
        for i in ids:
            ...
            for q in i:
                for j in paths:
                    if j[1] == q:
                        path.append(j[0])
            # add the initial position to the path list
